 ...
 * @className cn.eatboooo.study.week03.Demo05
 ...
 * 二叉树最远的两个节点，距离是多少
 */
public class Demo05_BT_MaxDistance {
    ...
    public static int maxDistance2(Node node) {
        return process1(node).maxDistance;
    }
    ...
    private static Info process1(Node node) {
        ...
        return new Info(Math.max(left.height, right.height) + 1, Math.max(Math.max(p1, p2), p3));
    }
